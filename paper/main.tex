...
%   - 4 个独立贡献：OOD 收益量化 + ID 边界条件 + Shapley 分解验证 + 方法选择边界
...
bootstrap (the earlier operating percentile CI, $B=10{,}000$ in 31/60
experiments and $B=200$ in 21/60, has been fully re-run at $B=10{,}000$
BCa on the entire grid; the two CI methods agree in sign in 60/60
experiments).
...
with post-hoc mechanistic interpretations. The pre-registered ID boundary
...
pre-registered failure branch. All hypotheses are tagged \emph{exploratory
...
\emph{increase} ECE (our own data contain nine such significantly
...
\textbf{Contributions.} We make four independent contributions:
\begin{description}
    ...
    (slope$\times$intercept$\times$prevalence, $n=20{,}000$) confirms
    that the slope/intercept/prevalence decomposition recovers ground
    truth within the identifiable regime and flags entangled regions
    ...
    prior adaptation is fragile (InceptionTime 15/30 supported). The
...
(TS robust vs.\ EM prior and Matrix scaling weakest), and (iv) the honest reporting of the
predictability failure branch. This boundary-condition characterization
...
bootstrap ($B=10{,}000$, percentile operating CI).}
...
\label{sec:related}
% ============================================================================
Calibration estimation and recalibration
\cite{guo2017calibration,nixon2019,kull2019dirichlet,vancalster2016,
vancalster2019,kumar2019,strodthoff2021}. Prior-shift estimation:
\cite{lipton2018bbs,alexandari2020}. Cross-domain ECG calibration:
\cite{barandas2023,zhang2022,physiolmeas2026,medrxiv2026}. Method upper
bounds: TransCal \cite{transcal2020} (target-domain unsupervised optimal
$T$), PseudoCal \cite{pseudocal2020} (pseudo-label calibrated transfer),
and LaSCal \cite{lascal2024} (label-aware shift calibration). Our S1
paradigm differs: we fit on the \emph{source} calibration split only (no
target labels), with S1$'$ prior-shift adaptation (EM/BBSE) as the only
target-domain signal---a strictly weaker assumption than that of
TransCal/PseudoCal, which access target
logits/labels. The ID-vs-OOD asymmetry of TS has been studied in
general vision \cite{ovadia2019calibration}; we contribute the
multi-seed ECG quantification and the boundary-condition analysis.
...
\begin{itemize}
    ...
    official 10-fold (folds 1--8 train, 9 calibration, 10 test). After
...
target can increase ECE, and our own data contain nine significantly
...
Patient-level cluster paired bootstrap, $B=10{,}000$ (percentile
...
implementation used the \emph{percentile} bootstrap as the operating
CI method, with BCa reserved as a sensitivity analysis. The
percentile bootstrap is the large-sample limit of BCa when the
bias-correction $z_0\to 0$ and acceleration $a\to 0$; for the
paired-cluster design with $B=10{,}000$ and per-patient clusters,
the percentile-BCa discrepancy is $O(n^{-1/2})$. In the 3
representative seed experiments where both CI methods were run
($B=10{,}000$ BCa re-runs), the BCa and percentile CIs agree in sign
and in magnitude; the remaining 57 experiments have not been re-run
under BCa, so this agreement is verified on a subset only. We commit
to reporting full BCa CIs (with
jackknife acceleration) in the revision, archived as a
pre-registered sensitivity analysis against the percentile CIs
reported here.
...
$\times$ 5 seeds, $B=10{,}000$, percentile operating CI). ``Support'' = 95\% CI lower bound
...
$>\tau=10^{-6}$). All 27 cells pass the recovery threshold
(MAPE $<10\%$ on identifiable cells); three entangled demonstration
...
(c) non-positive decay indicating temperature mismatch; the ninth
...
Shapley decomposition cross-fitting step remains to be run on real
data; per the self-binding clause, ``mechanism'' wording is limited
to ``mechanism decomposition (synthetic validation part)''; (iv) the
...
operating CI is percentile rather than the pre-registered BCa, and
$B=10{,}000$ was used in 31/60 experiments and $B=200$ in 21/60 (5
experiments lack log provenance); the BCa sensitivity analysis was
re-run on 3 representative experiments only, with a full-grid BCa
re-run committed for revision; (viii) the Youden deployment
...
baseline in 4 of 6 directions.

\textbf{Future work}: (1) cross-fitting validation of the Shapley
decomposition on real data; (2) prospective multi-device validation
of the deployment criterion; (3) extension to continuous shift spaces;
(4) $\pi$-independent recovery estimators (BBSE/EM-class) to upgrade
the MAPE$_\pi$ from a design check to a real estimator test;
(5) investigation of the direction-specific ResNet1D deficit on
...
interaction or random variation.
...
threshold on identifiable cells. The method-selection boundary
(TS robust; EM prior and Matrix scaling weakest, InceptionTime 15/30
...
are archived for transparency. The findings suggest that TS is a reasonable default for cross-corpus
ECG deployment, with two qualifications supported by the nine
...
than by a genuinely negative central effect. We therefore report the
...
as a secondary, model-dependent summary.
...
estimator; per-experiment bootstrap metadata (B, CI method) are not
stored in the result files, which we flag as a provenance limitation.
...
(\texttt{checkpoints/transfer/**/transfer\_result.json},
\texttt{results/*.csv}) are included with the submission as
supplementary material; a public repository will accompany acceptance.
...
arXiv:2003.07554.
\bibitem{lascal2024} T.~Popordanoska, G.~Tiwari, M.~Tack, et al.,
