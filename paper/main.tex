...
\textbf{Why this matters for ECG.} PTB-XL \cite{wagner2020ptbxl},
Chapman-Shaoxing \cite{zheng2022ecgarrhythmia}, and CPSC differ in
acquisition protocols, lead sets, and label vocabularies. A model
...
measurable shift, should I deploy the recalibrated model, buy labels, or
refuse?}

\textbf{Temperature scaling and pre-registration.}
...
\textbf{Contributions.} Five independent contributions:
\begin{description}
    ...
    with nine counter-examples disclosed and mechanistically explained.
    InceptionTime: three of six directions achieve 5/5, three achieve 4/5.
    ResNet1D: two of six achieve 5/5, two 4/5, two 3/5.
    \item[\textbf{C2} (Boundary condition, branch triggered):]
    ...
    \item[\textbf{C3} (Shapley validation):]
    ...
    \item[\textbf{C4} (Method selection):] Across
    ...
    ill-conditioned.
    \item[\textbf{C5} (Calibration gate):]
    ...
    pair$\times$architecture strata, 7/14 pass both layers; 7/14 fail
...
\textbf{Family structure}: BH-FDR $q=0.05$ over $6\text{ pairs}\times 2\text{ arch}\times 13\text{ shift levels}=156$
...
\textbf{Aggregate}: 51/60 supported (85.0\%; InceptionTime 27/30 $=$
...
\textbf{Stratified report by architecture.} The 60 experiments are
...
\textbf{Architecture variability.} Across the 30 TS experiments per
...
($R^2=-0.1424$). We run 2 architectures (InceptionTime + 1D-ResNet-34);
the BiMamba toy experiment ($n=60$) is deferred to the supplement.

\textbf{Result}: the merged LOO $R^2$ for TS is $-0.096$ with 95\% CI
...
\textbf{Sensitivity analysis with new features.} Re-running the LOO
...
\textbf{Sensitivity / specificity} (Table~\ref{tab:deploy_sens}):
...
\textbf{Deployment-time measurability.} Every lookup-table input is
...
\emph{Coverage note:} the L2 shift matrix covers 157 of the 390 planned
(pair, seed, shift) combinations (12 complete pair--seed combinations
$\times$ 13 levels, plus one partial seed-44 cell at fs250); seeds 42--43
cover all 6 InceptionTime pairs, 44--46 are pending, and ResNet1D L2 is
excluded, so these numbers rest on 2-seed, single-architecture coverage. The criterion is rarely satisfied, so TS is a useful default but
not a universally safe recalibrator, and should be combined with local
validation.

\textbf{Decision-curve analysis.} At the 10th-percentile raw-ECE
...
$+0.126$--$+0.164$. Observed raw ECE across our 60 seed experiments
ranges $0.06$ to $0.35$ (mean $0.15$), $4$--$13\times$ the raw ECE
...
architectures, PTB-XL$\rightarrow$Chapman having 3 including Mamba),
7/14 pass both layers and are deployable: CPSC$\rightarrow$PTB-XL,
...
7/14 fail Layer 2: Chapman$\rightarrow$PTB-XL,
...
\textbf{Result}: the OOD NCV is $+0.008125$ with
...
PTB-XL$\rightarrow$CPSC has zero). We disclose all nine: the protocol
requires all seed results, and each has an identifiable post-hoc mechanism
...
\textbf{Common pattern}: eight of nine counter-examples co-occur with
...
\emph{below} baseline in 4 of 6 directions: Chapman$\rightarrow$CPSC
0.510 vs.\ 0.528; CPSC$\rightarrow$PTB-XL 0.424 vs.\ 0.411
(essentially at baseline); CPSC$\rightarrow$Chapman 0.619 and
PTB-XL$\rightarrow$Chapman 0.620 (both well below 0.713). There
recalibration is moot, since no temperature improves a model that
should be replaced by a majority-class predictor, consistent with
the counter-example pattern (6/9 with low OOD accuracy).
Per-direction AUROC/AUPRC are not computed in the released artifacts
(accuracy only; a registered limitation, see Limitations (xiv)), so
the viability screen is accuracy-based; the discrimination-first gate
precedes the calibration decision rule in deployment.
...
improvement claim} (C5: 7/14 strata deployable) is partially qualified:
...
multi-threshold NCV analysis remain future work.

% ----------------------------------------------------------------------------
\subsection{Limitations and future work}
% ----------------------------------------------------------------------------
\textbf{Limitations}: (i) the main endpoint covers InceptionTime and
ResNet1D on all 6 directed pairs with 5-seed coverage (60 experiments);
BiMamba is a toy experiment ($n=60$, supplement) after memory exhaustion
on an RTX 5060 (8~GB); (ii) the predictability failure branch is
triggered (three-step to two-step); the deployment criterion is an
empirical lookup table; (iii) real-data Shapley cross-fitting
(\S\ref{sec:real_shapley}) shows 85\% direction accuracy but magnitude
overestimation (mean error $+0.29$); $T$, Wasserstein-1, and entropy
features confirm the boundary (all $R^2 < 0$); (iv) the 27-cell
decomposition validation uses binned ECE, not the main endpoint's
SmoothECE, limiting cross-definition use; (v) the safety criterion is
rarely satisfied on the full L2 shift matrix (TS safety rate $0.0064$);
TS is a useful default, not universally safe; (vi) nine counter-examples
of 60 experiments (15.0\%) are disclosed but not excluded; (vii) the
operating CI is BCa at $B=10{,}000$, re-run on all 60 experiments
(60/60, \texttt{methods.ts.meta}); percentile CIs from $B=200$ in 21/60
experiments remain (5 lack logs); (viii) the Youden threshold is fitted
in-sample, making the operating characteristic optimistic; (ix) the L2
analysis covers 157 of 390 planned cells at 2 seeds on one architecture,
with synthetic noise rather than the unavailable pre-registered PTB-XL
Database; (x) temperature was fitted once per experiment, so CIs omit
temperature-fitting uncertainty (the two-layer bootstrap is disabled);
(xi) model selection used validation ECE rather than the pre-registered
...
AUPRC, so the viability precondition is screened by accuracy alone, with
OOD accuracy below the majority-class baseline in 4 of 6 directions;
...
pre-registration protocol and public repository.  \textbf{Future work}:
...
each). \textbf{(C5)} Gate: 7/14 strata pass both layers, reframing the 9
...
Nine counter-examples are disclosed, mostly with low OOD accuracy ($\leq
0.50$ in 6/9) and a large ID--OOD gap ($\geq 0.18$ in 7/9), a tendency
rather than a sufficient gate (three have OOD accuracy $>0.5$). TS needs
local validation (safety rate $0.0064$), the Youden criterion is fitted
in-sample, and OOD accuracy is below the majority-class baseline in 4 of
6 directions, where TS cannot help.  A noise floor control
(\S\ref{sec:noise_floor}) rules out a SmoothECE artifact: worst-case
metric noise ($+0.021$) is $4$--$13\times$ below the raw ECE
($0.06$--$0.35$). The C5 gate (\S\ref{sec:gate}) rejects every
clinically moot stratum. Real-data Shapley cross-fitting
(\S\ref{sec:real_shapley}) gives 85\% direction accuracy with magnitude
overestimation (mean error $+0.29$); $T$, Wasserstein-1, and entropy
features confirm the boundary (all $R^2 < 0$).  \textbf{Meta-analytic
pooling.} The fixed-effect estimate is CI-width sensitive: ResNet1D is
slightly negative ($-0.000573$) because narrow-CI studies dominate,
while DerSimonian--Laird random effects are positive (InceptionTime
$+0.0151$, ResNet1D $+0.0158$, CrossArchitecture $+0.0148$; unweighted
$+0.0152$, $+0.0165$, $+0.0159$). This reflects heterogeneity ($\tau^2
\approx 3\times10^{-5}$, $Q \gg k-1$), not a negative central effect, so
the count-based proportion (51/60 = 85.0\%) is primary; $\tau^2$ varies
by 41\% (pooled estimate stable within 3.3\%).  \textbf{Family size.}
The 60 experiments form the robustness-evidence family; the confirmatory
family is the $6 \times 2 \times 13 = 156$ shift grid, and the $8 \times
6 \times 5 \times 2 = 480$-cell method design is in
Sec.~\ref{sec:method_boundary}. BH-60 and Bonferroni leave the headline
unchanged (51/60); stratum aggregation (12 strata) retains 9/12
directions (InceptionTime 6/6), and Bonferroni-12 retains 2/12
(cpsc\_chapman/IT, ptbxl\_chapman/RN).  \textbf{CI width.} Across the 60 TS experiments the 95\% CI widths span
...
excluding those the count becomes 45/60 (75.0\%; InceptionTime 24/30,
ResNet1D 21/30). Narrow CIs reflect the moderate calibration sample
($n_{\text{cal}} = 1027$--$2099$ records, $1027$--$1873$ patients) and low
SmoothECE variance ($B=10{,}000$, CI method=BCa); they do not affect the
sign-based support/counter-example classification.
...
This study used only publicly available databases: PTB-XL
(v1.0.3, PhysioNet), Chapman--Shaoxing (PhysioNet), and CPSC2018+2019
(PhysioNet Challenge). No new raw waveforms were collected. The patient-level
split indices generated in this work, the label-mapping tables, the analysis
code, and the per-experiment result artifacts are openly available at
\url{https://github.com/gt17641001169-design/ecg-calibration-transportability};
trained model checkpoints are provided as release assets of the same
repository.
